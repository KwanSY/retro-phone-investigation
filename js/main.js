...
function updatePointerPos(e) {
    ...
    if (clientX !== undefined && clientY !== undefined) {
        if (state.scale) {
            state.mouseX = (clientX - state.offsetX) / state.scale;
            state.mouseY = (clientY - state.offsetY) / state.scale;
        } else {
...
canvas.addEventListener('touchstart', handleClickEvent, { passive: true });
