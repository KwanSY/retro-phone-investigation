...
/**
 * Stop any currently playing or scheduled phone ringing sounds immediately
 */
export function stopPhoneRingSound() {
    if (!activeRingNodes || activeRingNodes.length === 0) return;
    const ctx = getAudioContext();
    const now = ctx ? ctx.currentTime : 0;

    activeRingNodes.forEach(item => {
        try {
            if (item.gain && ctx) {
